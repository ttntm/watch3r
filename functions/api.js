  ...
  if (!claims) {
    ...
    switch (event.httpMethod) {
      case 'GET':
        ...
        return event.list && event.user
          ? api.read(event, context)
          : pathError

      case 'POST':
        // target = listname
        return event.target
          ? api.create(event, context)
          : pathError

      case 'PUT':
        // Update item in table based on itemid + payload
        [event.list, event.listItemId] = getMethodPath(event.path)

        return event.list && event.listItemId
          ? api.update(event, context)
          : pathError

      case 'DELETE':
        // Delete itemid from table, table = event.target, itemid = payload
        return event.target
