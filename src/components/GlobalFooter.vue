...
<template>
  <div class="text-sm px-4 py-2 sm:py-4 mt-8">
    <nav class="hidden sm:flex footer-nav flex-row flex-wrap justify-center items-center">
      <router-link v-for="item in menuItems" :key="item.name" v-click-blur :to="{name: item.name}">
        {{ item.name }}
      </router-link>
    </nav>
    <BtnMenu
      :iconSize="24"
      class="flex sm:hidden flex-row items-center justify-center text-sm font-bold text-yellow-600 hover:text-gray-900 mx-auto"
    >
      Show Menu
    </BtnMenu>
