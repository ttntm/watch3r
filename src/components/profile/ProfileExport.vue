...
<template>
  ...
  <div class="flex flex-row items-center text-sm mb-4">
    ...
    <InputRadio class="mr-4" name="export-mode" :label="'watchlist'" :value="exportMode" @update:radio="events.onUpdateExportMode($event)" />
  ...
  <div class="flex flex-row items-center text-sm">
    ...
    <InputRadio class="mr-4" name="export-format" :label="'CSV'" :value="exportFormat" @update:radio="events.OnUpdateExportFormat($event)" />
    <InputRadio class="" name="export-format" :label="'JSON'" :value="exportFormat" @update:radio="events.OnUpdateExportFormat($event)" />
  </div>
  <div class="h-px bg-gray-500 mt-6 mb-8" />
  <button v-click-blur class="btn btn-black flex flex-row items-center justify-center" :disabled="listLength === 0" @click="events.onBtnExportClick">
