        ...
        complete: results => {
          ...
          if (parsed.length > 0 && parsed[0].Const) {
            ...
            store.dispatch('app/sendToastMessage', {
              text: `Invalid data in the selected CSV. Please try exporting from IMDb again.`,
              type: 'error'
            })
          }
        },
...
<template>
  <div id="import">
    <div class="w-full sm:w-2/3 lg:w-1/2 sm:text-center mx-auto">
      ...
      <div v-if="importList.length > 0" class="flex flex-row items-center justify-center text-sm my-8">
        ...
        <InputRadio
          ...
          class="mr-4"
          name="import-mode"
          @update:radio="events.onUpdateMode($event)"
        />
        <InputRadio
          :label="'tracklist'"
          :value="importMode"
    ...
    <div v-if="importList.length > 0" class="list w-full xl:w-2/3 mx-auto">
      <ListItemImport
        v-for="item in importList"
        :key="item.Const"
        :item="item"
        :mode="importMode"
        @import-title="events.onImportTitle($event)"
      />
    </div>
  </div>
